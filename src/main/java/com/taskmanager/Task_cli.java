    ...
    public static void main(String[] args) throws SQLException {
        ...
        try {
            ...
            switch (command) {
                ...
                case "update" -> {
                    // user has to give the index and the description, without any description
                    // there's no command
                    if (args.length >= 3) {
                ...
                // mark a task in a new status, by giving the number and the new status to the
                // arguments
                case "mark" -> {
                    if (args.length >= 3) {
